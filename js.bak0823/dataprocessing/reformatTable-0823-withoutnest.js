...
var routeCollection = {
    ...
    yearForCountriesList : [ ],
    yearForCountriesListCount : [ ],
    routesForCountries: [],
...
function readFlowsByYear(){
    ...
    csv().fromFile('../../data/TBL_ship_final.csv').on('json', function (item) {
        ...
        fswirting.writeFile('../../data/flowsByYear.json', JSON.stringify(routeCollection, null, 4), function (err) {
            if (err) {
                console.log(err);
            } else {
